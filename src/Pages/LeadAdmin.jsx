...
function LeadAdmin() {
  ...
  const applyFilters = (userEmail, searchValue) => {
    ...
    if (userEmail) filtered = filtered.filter((lead) => lead.email === userEmail)
    if (searchValue)
      ...
      <div className="w-[38%] border-r border-gray-100 p-4 bg-white flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-2 scrollbar-thin scrollbar-thumb-transparent scrollbar-track-transparent hover:scrollbar-thumb-gray-300 hover:scrollbar-track-gray-100">
          {loading ? (
            ...
            leads.map((lead) => (
              <LeadCard key={lead._id} lead={lead} users={users} onSelect={setSelected} selected={selected?._id === lead._id} />
            ))
          )}
        </div>
      </div>
      <div className="w-[65%] flex flex-col">
        <LeadAdminDetails selected={selected} users={users} role={role} setShowFollowUp={setShowFollowUp} showNotes={showNotes} setShowNotes={setShowNotes} />
        {showFollowUp && selected && <FollowUpPopup lead={selected} onClose={() => setShowFollowUp(false)} />}
      </div>
    </div>
  )
}
